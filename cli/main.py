...
class MessageBuffer:
    ...
    FIXED_AGENTS = {
        "研究团队": ["看多研究员", "看空研究员", "研究经理"],
        "交易团队": ["交易员"],
        "风险管理": ["激进型分析师", "中性分析师", "保守型分析师"],
        "投资组合管理": ["投资组合经理"],
    }
    ...
    REPORT_SECTIONS = {
        ...
        "sentiment_report": ("social", "社交舆情分析师"),
        "news_report": ("news", "新闻分析师"),
        "fundamentals_report": ("fundamentals", "基本面分析师"),
        "investment_plan": (None, "研究经理"),
        "trader_investment_plan": (None, "交易员"),
        "final_trade_decision": (None, "投资组合经理"),
    }
    ...
    def _update_final_report(self):
        ...
        if self.report_sections.get("trader_investment_plan"):
            report_parts.append("## 交易计划")
            report_parts.append(f"{self.report_sections['trader_investment_plan']}")

        # Portfolio Management Decision
        if self.report_sections.get("final_trade_decision"):
            report_parts.append("## 投资组合决策")
...
def update_display(layout, spinner_text=None, stats_handler=None, start_time=None):
    ...
    # Build stats parts
    stats_parts = [f"角色: {agents_completed}/{agents_total}"]
...
def save_report_to_disk(final_state, ticker: str, save_path: Path):
    ...
    if final_state.get("sentiment_report"):
        ...
        analyst_parts.append(("社交舆情分析师", final_state["sentiment_report"]))
    ...
    if final_state.get("risk_debate_state"):
        ...
        if risk.get("aggressive_history"):
            ...
            risk_parts.append(("激进型分析师", risk["aggressive_history"]))
        if risk.get("conservative_history"):
            risk_dir.mkdir(exist_ok=True)
            (risk_dir / "conservative.md").write_text(risk["conservative_history"])
            risk_parts.append(("保守型分析师", risk["conservative_history"]))
...
def display_complete_report(final_state):
    ...
    if final_state.get("sentiment_report"):
        analysts.append(("社交舆情分析师", final_state["sentiment_report"]))
    if final_state.get("news_report"):
    ...
    # III. Trading Team
    if final_state.get("trader_investment_plan"):
        console.print(Panel("[bold]III. 交易计划[/bold]", border_style="yellow"))
    ...
    if final_state.get("risk_debate_state"):
        ...
        risk_reports = []
        if risk.get("aggressive_history"):
            risk_reports.append(("激进型分析师", risk["aggressive_history"]))
        if risk.get("conservative_history"):
            risk_reports.append(("保守型分析师", risk["conservative_history"]))
        if risk.get("neutral_history"):
            risk_reports.append(("中性分析师", risk["neutral_history"]))
        if risk_reports:
            console.print(Panel("[bold]IV. 风险管理团队结论[/bold]", border_style="red"))
...
def update_research_team_status(status):
    ...
    research_team = ["看多研究员", "看空研究员", "研究经理"]
...
def update_analyst_statuses(message_buffer, chunk):
    ...
    if not found_active and selected:
        if message_buffer.agent_status.get("看多研究员") == "pending":
            message_buffer.update_agent_status("看多研究员", "in_progress")
...
def run_analysis():
    ...
    with Live(layout, refresh_per_second=4) as live:
        ...
        message_buffer.add_message("System", f"已选股票代码：{selections['ticker']}")
        ...
        for chunk in graph.graph.stream(init_agent_state, **args):
            ...
            if chunk.get("investment_debate_state"):
                ...
                if bull_hist:
                    message_buffer.update_report_section(
                        "investment_plan", f"### 看多研究员观点\n{bull_hist}"
                    )
                if bear_hist:
                    message_buffer.update_report_section(
                        "investment_plan", f"### 看空研究员观点\n{bear_hist}"
                    )
                if judge:
                    message_buffer.update_report_section(
            ...
            if chunk.get("trader_investment_plan"):
                ...
                if message_buffer.agent_status.get("交易员") != "completed":
            ...
            if chunk.get("risk_debate_state"):
                ...
                if agg_hist:
                    if message_buffer.agent_status.get("激进型分析师") != "completed":
                        message_buffer.update_agent_status("激进型分析师", "in_progress")
                    message_buffer.update_report_section(
                        "final_trade_decision", f"### 激进型分析师观点\n{agg_hist}"
                    )
                if con_hist:
                    if message_buffer.agent_status.get("保守型分析师") != "completed":
                        message_buffer.update_agent_status("保守型分析师", "in_progress")
                    message_buffer.update_report_section(
                        "final_trade_decision", f"### 保守型分析师观点\n{con_hist}"
                    )
                if neu_hist:
                    if message_buffer.agent_status.get("中性分析师") != "completed":
                        message_buffer.update_agent_status("中性分析师", "in_progress")
                    message_buffer.update_report_section(
                        "final_trade_decision", f"### 中性分析师观点\n{neu_hist}"
                    )
                if judge:
                    if message_buffer.agent_status.get("投资组合经理") != "completed":
                        message_buffer.update_agent_status("投资组合经理", "in_progress")
                        message_buffer.update_report_section(
                            "final_trade_decision", f"### 投资组合经理决策\n{judge}"
                        )
                        message_buffer.update_agent_status("激进型分析师", "completed")
                        message_buffer.update_agent_status("保守型分析师", "completed")
